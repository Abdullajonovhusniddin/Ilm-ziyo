  ...
  return (
    <div>
      <div className={styles.pageHeader} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 className={styles.pageTitle}>{t("Moliya nazorati boʻlimi")}</h1>
          <p className={styles.pageDescription}>{t("Kirim va chiqimlarni nazorat qilish.")}</p>
        </div>
        <button 
      ...
      <div className={styles.cardContainer}>
        <div className={styles.card}>
           ...
           <h3>{t("Umumiy kirim")}</h3>
           <p style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--success)', marginTop: '0.5rem' }}>{t("+ 450 mln")}</p>
        </div>
        <div className={styles.card}>
           <div className={styles.cardIcon}><TrendingDown size={24} color="var(--danger)" /></div>
           <h3>{t("Umumiy chiqim")}</h3>
           <p style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--danger)', marginTop: '0.5rem' }}>{t("- 120 mln")}</p>
        </div>
        <div className={styles.card}>
           <div className={styles.cardIcon}><ArrowUpRight size={24} color="var(--primary)" /></div>
           <h3>{t("Sof foyda")}</h3>
           <p style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--primary)', marginTop: '0.5rem' }}>{t("330 mln")}</p>
        </div>
      </div>

      <SlideDrawer isOpen={isDrawerOpen} onClose={() => setDrawerOpen(false)} title={t("Xarajat yoki Kirim qo'shish")}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <label style={{ fontSize: '0.875rem', fontWeight: '500', color: 'var(--secondary)' }}>{t("Turi")}</label>
            <select style={{ padding: '0.75rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', background: 'var(--background)', color: 'var(--text-main)', outline: 'none' }}>
              <option>{t("Kirim")}</option>
              <option>{t("Chiqim")}</option>
            </select>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <label style={{ fontSize: '0.875rem', fontWeight: '500', color: 'var(--secondary)' }}>{t("Summa (so'm)")}</label>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <label style={{ fontSize: '0.875rem', fontWeight: '500', color: 'var(--secondary)' }}>{t("Izoh")}</label>
